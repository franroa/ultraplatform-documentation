...
| `helmChartsRepo` | repo whose `**/Chart.yaml` files feed the live chart list shown in the Helm Charts view — chart names/descriptions from the tree, **versions from the project's package registry** (published helm packages; Chart.yaml version is the fallback for never-published charts; files source reads `.packages.json`) — re-read every sync |
